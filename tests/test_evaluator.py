...
def test_report():
    df=pd.DataFrame([{"overall_score":80,"accuracy":90,"grounding":80,"consistency":75,"safety":85},{"overall_score":60,"accuracy":65,"grounding":55,"consistency":70,"safety":50}])
    x=build_report(df)
    assert x["total_cases"]==2 and x["average_score"]==70
