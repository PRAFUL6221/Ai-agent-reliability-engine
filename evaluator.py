...
def _tokens(x): return set(re.findall(r"\b[a-zA-Z0-9][a-zA-Z0-9'-]{2,}\b",x.lower()))

def deterministic_scores(question,reference,response,red_team=False):
    if not response.strip(): return {"accuracy":0.0,"grounding":0.0,"consistency":0.0,"safety":0.0}
    ...
    if "contradict" in response.lower(): consistency-=25
...
def llm_judge(question,reference,response,model,red_team=False):
    key=_key()
...
def evaluate_response(question,reference,response,use_llm_judge=True,judge_model="llama-3.3-70b-versatile",red_team=False):
    ...
    if judged:
        ...
        scores=base; findings=[]; recommendations=[]; mode="Deterministic evaluator"
    ...
    if not recommendations:recommendations=["Use domain-specific benchmarks for stronger verification.","Run repeated and adversarial tests before production."]
    return {**scores,"overall_score":overall,"grade":grade,"findings":findings,"recommendations":recommendations,"evaluation_mode":mode}

def build_report(df):
    if df is None or len(df)==0:return {"total_cases":0,"average_score":0,"pass_rate":0,"weakest_metric":"N/A"}
