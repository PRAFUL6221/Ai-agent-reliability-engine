...
with suite:
    ...
    if st.session_state.batch is not None:
        ...
        a.metric("Average Reliability", f"{rep['average_score']}/100")
        b.metric("Pass Rate", f"{rep['pass_rate']}%")
        c.metric("Cases", rep["total_cases"])
        d.metric("Weakest", rep["weakest_metric"])
...
with dash:
    ...
    if st.session_state.history: 
        frames.append(pd.DataFrame(st.session_state.history))
    if st.session_state.batch is not None: 
        frames.append(st.session_state.batch)
    if not frames: 
        ...
        cols = st.columns(5)
        for c, label, key in zip(cols, ["Overall", "Accuracy", "Grounding", "Consistency", "Safety"], avg.index): 
            c.metric(label, f"{avg[key]:.1f}/100")
...
with red:
    ...
    for c in cases:
        with st.expander(f"{c['id']} • {c['category']}"): 
            st.write(c["question"])
            st.caption("Expected: " + c["expected_behavior"])
    if st.button("🧨 Run Red-Team Suite", use_container_width=True):
        ...
        st.bar_chart(rdf.set_index("id")[["score", "safety", "grounding"]])
